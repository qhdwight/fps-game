...
public class Player : NetworkBehaviour
{
    ...
    private const float MAX_HEIGHT = 256F, MIN_HEIGHT = -64F;
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.J) && !isDead && isLocalPlayer)
        {
            CmdTakeDamage(20);
        }

        bool below = transform.position.y < MIN_HEIGHT;
        bool above = transform.position.y > MAX_HEIGHT;
        if ((below || above) && !isDead)
        {
            isDead = true;
            GetComponent<Rigidbody>().position = Vector3.zero;
            GetComponent<Rigidbody>().velocity = Vector3.zero;
            string deathString = below ? "void" : "zues";
            CmdSuicide(deathString);
        }
    }
    ...
    {
        RpcTakeDamage(amount, netId.Value, false, "testerino");
    }
    ...
    [Command]
    private void CmdSuicide(string death)
    {
        // Add death to player
        deaths++;
        // Create kill feed
    ...
    [ClientRpc]
    public void RpcSetHealth(int val)
    {
        currentHealth = val;
    }
    ...
    [Server]
    public IEnumerator HealOverTime(int healthPerSecond)
    {
        while (!isDead)
        {
            yield return new WaitForSeconds(1F);
            if (currentHealth + healthPerSecond < maxHealth)
                RpcHeal(healthPerSecond);
            else
                RpcSetHealth(maxHealth);
        }
    }
    ...
    public void SetHitboxLayer(string layerName)
    {
        foreach (Collider col in colliders)
        {
